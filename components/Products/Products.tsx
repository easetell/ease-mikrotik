        ...
        if (product.stock < 100) {
          try {
            ...
            if (response.status === 200) {
              ...
              toast.error(
                `Failed to send notification for ${product.productName}`,
              );
            }
          } catch (error) {
            toast.error(
              `Error sending notification for ${product.productName}`,
            );
          }
        }
      });
                ...
                <tr key={productItem._id}>
                  ...
                  <td className="border-b border-[#eee] px-4 py-5 dark:border-dark-3">
                    <p className="text-dark dark:text-white">
                      Ksh. {Intl.NumberFormat().format(productItem.price)}
                    </p>
                  </td>
                  <td className="border-b border-[#eee] px-4 py-5 dark:border-dark-3">
                    <p className="text-dark dark:text-white">
                  ...
                  <td className="space-x-2 whitespace-nowrap border-b border-[#eee] p-4 px-4 py-5 dark:border-dark-3">
                    ...
                    <button
                      type="button"
                      onClick={() => handleDeleteButtonClick(productItem._id)}
